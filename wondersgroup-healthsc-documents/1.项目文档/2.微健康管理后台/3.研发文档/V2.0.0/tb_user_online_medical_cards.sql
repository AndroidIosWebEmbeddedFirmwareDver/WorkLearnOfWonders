...
CREATE TABLE `tb_user_online_medical_cards` (
  ...
  `card_type_code` varchar(32) DEFAULT NULL COMMENT '在线挂号卡类型编码描述',
  `card_type_name` varchar(32) CHARACTER SET utf8 DEFAULT NULL COMMENT '在线挂号卡类型名称描述',
  `hospital_id` bigint(11) DEFAULT '0' COMMENT '医院ID；如果卡类型是就诊卡则不为空',
  `hospital_code` varchar(32) CHARACTER SET utf8 DEFAULT NULL COMMENT '医院代码；如果卡类型是就诊卡则不为空',
  `hospital_name` varchar(32) CHARACTER SET utf8 DEFAULT NULL COMMENT '医院名称；如果卡类型是就诊卡则不为空',
...
) ENGINE=InnoDB AUTO_INCREMENT=241 DEFAULT CHARSET=utf8mb4 COMMENT='文章表';

...
- (void)setupView {
    ...
    self.contentLabel1 = [UISetupView setupTextFieldWithSuperView:self.contentView withText:@"" withTextColor:[UIColor tc1Color] withFontSize:16 withPlaceholder:@"(必填)" withDelegate:self withReturnKeyType:UIReturnKeyDone withKeyboardType:UIKeyboardTypeDefault];
...
-(void)setViewModel:(SCAppointmentDetailViewModel *)viewModel {
    ...
    [RACObserve(viewModel, isNotInputCard) subscribeNext:^(NSNumber *x) {
        if ([x boolValue]) {
            weakSelf.contentView.backgroundColor = [UIColor sbc6Color];
        }else {

...
import static com.wondersgroup.healthcloud.jpa.entity.pay.PayOrder.Status.NOTPAY;
 ...
 * @author nick
 */
public class NotPayOrderService extends AbstractPayOrderService {

    private PayService payService;

    public NotPayOrderService(int start, int end, PayOrder.Status status, PayService payService) {
    ...
    public MyOrder getMyOrders(String uid) {
        ...
        int total = payService.countByUidAndStatus(uid, NOTPAY.toString());
        if (total == 0) {
            hasMore = false;
        } else {
            ...
            payOrders = payService.findByUidAndStatus(uid, NOTPAY.toString(), start, pageSize);
        }
    ...
    public MyOrder getMyOrders(String uid, String cityCode) {
        ...
        boolean hasMore = true;
        List<PayOrder> payOrders = Lists.newArrayList();
        int total = payService.countByUidAndStatus(uid, NOTPAY.toString(), cityCode);
        if (total == 0) {
            hasMore = false;
        } else {
            if (start + pageSize >= total) {
            ...
            payOrders = payService.findByUidAndStatus(uid, NOTPAY.toString(), start, pageSize, cityCode);
        }
        myOrder.setOrders(generateOrders(payOrders, payService.hospitalService));
        myOrder.setNextPage(String.valueOf(start + pageSize));
        myOrder.setHasMore(hasMore);

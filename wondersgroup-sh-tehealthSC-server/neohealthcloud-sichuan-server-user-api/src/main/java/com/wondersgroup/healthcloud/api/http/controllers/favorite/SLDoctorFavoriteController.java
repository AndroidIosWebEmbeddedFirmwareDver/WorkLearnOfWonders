 ...
 */
@RestController
@RequestMapping("/api/sl/favorite/doctor")
public class SLDoctorFavoriteController {
    ...
    @GetMapping("/list")
    @VersionRange
    public JsonListResponseEntity<DoctorInfoDTO> list(@RequestHeader(name = "city-code", defaultValue = "510122000000") String cityCode,
        ...
        List<Doctor> doctors = favoriteService.getFavoriteDoctors(uid, flag, 10, cityCode);
        List<DoctorInfoDTO> doctorInfo = departmentService.getDoctorInfo(doctors, cityCode);
        Boolean hasMore = false;
        if (doctors != null && doctors.size() > 10) {

...
- (NSArray *)tabbarViewControllers {
    ...
    return @[@[@"HomePageViewController", @"首页", self.tabModel.tabImages[0][0], self.tabModel.tabImages[0][1]],
             @[@"HealthHomeViewController", @"健康", self.tabModel.tabImages[1][0], self.tabModel.tabImages[1][1]],
             @[@"SCSystemViewController", @"消息", self.tabModel.tabImages[1][0], self.tabModel.tabImages[2][1]],
             @[@"WDProfileViewController", @"我的", self.tabModel.tabImages[2][0], self.tabModel.tabImages[3][1]],
//              @[@"ReportListViewController", @"我的", self.tabModel.tabImages[2][0], self.tabModel.tabImages[2][1]],
             ];
}
...
- (void)getLocalTablImages {
    ...
    if([defaults objectForKey: TAB_IMAGE_KEY]) {
        ...
        NSArray *list    = @[@[dic[@"tab1Sel"], dic[@"tab1"]],
                             ...
                             @[dic[@"tab3Sel"], dic[@"tab3"]],
                             @[dic[@"tab4Sel"], dic[@"tab4"]],
                             @[dic[@"tabBg"]]];
    ...
    [self.adapter request: UI_TAB_IMAGE params: nil class: nil responseType:Response_Object method:Request_GET needLogin: NO success:^(NSURLSessionDataTask *task, ResponseModel *response) {
        ...
        if (imageList.count == 9) {
            self.tabModel.tabImages = @[@[imageList[5],imageList[1]],
                                        ...
                                        @[imageList[7],imageList[3]],
                                        @[imageList[8],imageList[4]],
                                        @[imageList[0]]
                                        ];
        }

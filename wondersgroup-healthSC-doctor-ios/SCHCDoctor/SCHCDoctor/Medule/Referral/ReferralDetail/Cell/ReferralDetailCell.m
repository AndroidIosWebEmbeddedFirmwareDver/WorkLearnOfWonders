...
- (void)setupView {
    ...
    _referralStatusLabel = [[UILabel alloc] init];
    [_referralStatusLabel.layer setBorderWidth:1];
    [_referralStatusLabel.layer setCornerRadius:8];
    
    _urgencyLabel = [[UILabel alloc] init];
    [_urgencyLabel.layer setBorderWidth:1];
    [_urgencyLabel.layer setCornerRadius:8];
    
    
    
    
    _outTitleLabel = [self getTitleLabel];
    [_outTitleLabel setText:@"转出:"];
    [_outTitleLabel setFont:[UIFont fontWithName:@"PingFangSC-Regular" size:14]];
    _inTittlelabel = [self getTitleLabel];
    [_inTittlelabel  setText:@"转入:"];
...
//    [_detailButton rj_leftTextRightImage];
    ...
    NSArray * views = @[_typeImageView,_typeLabel,_timeLabel,_topLineLabel,_outTitleLabel,_inTittlelabel,_outHospitalLabel,_inHospitalLabel,_centerLineLabel,_detailButton,_detailContentView,_urgencyLabel, _referralStatusLabel];
...
- (void)buildConstraint {
    ...
    [_urgencyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_referralStatusLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(_urgencyLabel);
        make.left.equalTo(_urgencyLabel.mas_right).offset(5);
        make.height.mas_equalTo(16);
        make.width.mas_greaterThanOrEqualTo(0);
    ...
    [_centerLineLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_greaterThanOrEqualTo(_outHospitalLabel.mas_bottom).offset(20);
    }];
...
- (void)setData:(ReferralDetailReferralModel *)data {
    if ([data.upOrDown isEqualToString:@"上转"]) {
        [_typeImageView setImage:[UIImage imageNamed:@""]];
    } else {
        [_typeImageView setImage:[UIImage imageNamed:@""]];
    }
    
    [_typeLabel setText:data.upOrDown];
    [_urgencyLabel setText:data.urgency];
    [_referralStatusLabel setText:data.referralStatus];
    [_timeLabel setText:data.referralDate];
    [_outHospitalLabel setText:data.fromOrgName];
    [_inHospitalLabel setText:data.toOrgName];
    [_reasonDetailLabel setText:data.referralReson];
    [_checkDetailLabel setText:data.tentativeDiagnosis];
    [_historyDetailLabel setText:data.medicalHistory];
    [_infoDetailLabel setText:data.pastHistory];
    [_cureDetailLabel setText:data.treatment];

...
import com.wonders.health.venus.open.user.module.consultation.db.ChatDBManager;
import com.wonders.health.venus.open.user.module.consultation.db.InviteMessgeDao;
...
import com.wondersgroup.hs.healthcloud.common.huanxin.model.EaseNotifier;
...
public class ChatHelper {
    ...
    private Map<String, EaseUser> chatAvatorList;
...
//        options.setMipushConfig("2882303761517426801", "5381742660801");
        //华为推送appid
//        options.setHuaweiPushAppId("100018755");
...
//                EaseEmojiconGroupEntity data = EmojiconExampleGrou.getData();
...
//                }
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotifier.EaseNotificationInfoProvider() {
            ...
            public String getTitle(EMMessage message) {
                // 修改标题,这里使用默认
                try {
                    return message.getStringAttribute(ChatConstant.EXTRA_USER_NAME);
                } catch (Exception e) {
                    return null;
                }
            }
            ...
            public String getDisplayedText(EMMessage message) {
                ...
                String userName = "";
                try {
                    userName = message.getStringAttribute(ChatConstant.EXTRA_USER_NAME);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            public String getLatestText(EMMessage message, int fromUsersNum, int messageNum) {
                ...
                if (body instanceof EMTextMessageBody) {
                    if (messageNum == 1) {
                        ...
                        return "[" + messageNum + "条]" + userName + ":" + ((EMTextMessageBody) body).getMessage();
                    }
                } else if (body instanceof EMImageMessageBody) {
                    if (messageNum == 1) {
                        return "[图片]";
                    } else {
                        return "[" + messageNum + "条]" + userName + ":[图片]";
                    }
                } else if (body instanceof EMVoiceMessageBody) {
                    if (messageNum == 1) {
                        return "[语音]";
                    } else {
                        return "[" + messageNum + "条]" + userName + ":[语音]";
                    }
                } else {
                    return null;
                }
            ...
            public Intent getLaunchIntent(EMMessage message) {
                //设置点击通知栏跳转事件
                Intent intent = new Intent(appContext, MyChatActivity.class);
...
//                    intent = new Intent(appContext, VoiceCallActivity.class);
                    ...
                    if (chatType == ChatType.Chat) { // 单聊信息
                        intent.putExtra(EaseConstant.EXTRA_CHAT_TO_USER_ID, message.getFrom());
                        intent.putExtra(EaseConstant.EXTRA_CHAT_TYPE, EaseConstant.CHATTYPE_SINGLE);
                    } else { // 群聊信息
                        // message.getTo()为群聊id
                        intent.putExtra(EaseConstant.EXTRA_CHAT_TO_USER_ID, message.getTo());
...
//                    EventBus.getDefault().post(new EmConnectionConflictEvent());
                    LogUtils.d("easemob 环信已断开连接");
...
//        appContext.registerReceiver(callReceiver, callFilter);
    ...
    private void initDbDao() {
        ...
        userDao = new ChatUserDao(appContext);
    }
...
//            return getUserProfileManager().getCurrentUserInfo();
        user = getChatAvatorList().get(username);
...
//        }
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    //sun add
                    EasemobUnreadMsgChangeEvent event = new EasemobUnreadMsgChangeEvent();
     ...
     * @param talkPwd 环信密码
     */
    public void login(String talkId, String talkPwd, final EMCallBack callBack) {
        // 调用sdk登录方法登录聊天服务器
        if(TextUtils.isEmpty(talkId)||TextUtils.isEmpty(talkPwd)){
            Log.d("main", "环信用户名或密码为空！");
            return;
        }
        EMClient.getInstance().login(talkId, talkPwd,new EMCallBack() {//回调
            @Override
...
//		endCall();
     ...
     * @return
     */
    public Map<String, EaseUser> getChatAvatorList() {
        // return a empty non-null object to avoid app crash
        if (chatAvatorList == null) {
            chatAvatorList = new Hashtable<String, EaseUser>();
        }
        return chatAvatorList;
    }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    // 存入db
                    ChatUserDao dao = new ChatUserDao(appContext);

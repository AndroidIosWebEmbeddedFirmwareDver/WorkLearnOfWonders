...
-(void)setupView {
    ...
    UIButton *submitBtn = [UISetupView setupButtonWithSuperView:btnView withTitleToStateNormal:@"提交" withTitleColorToStateNormal:[UIColor tc0Color] withTitleFontSize:16 withAction:^(UIButton *sender) {
        [weakSelf submit];
    }];
...
//    }
    ...
    [self.viewModel submitAppointmentDetail:^(WDRegistrationPayModel *model){
        ...
        vc.viewModel.registrationOrder = model;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            static NSString *identifier = @"SCAppointmentDetailRuleCell";
            
            SCAppointmentDetailRuleCell * cell = [tableView dequeueReusableCellWithIdentifier:identifier];
            if(!cell)
            {
                cell = [[SCAppointmentDetailRuleCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
                cell.lineTopHidden = YES;
        ...
        if (indexPath.row == 0 || indexPath.row == 1 || indexPath.row == 3) {
            ...
            static NSString *identifier = @"SCAppointmentDetailInputCell";
            
            SCAppointmentDetailInputCell * cell = [tableView dequeueReusableCellWithIdentifier:identifier];
            if(!cell)
            {
                cell = [[SCAppointmentDetailInputCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            return [SCAppointmentDetailRuleCell cellHeight];
        }
    }else {

app.controller('helpCenterEditController',['$scope','$http','$state','$stateParams',function($scope, $http, $state,$stateParams){
    $scope.questionObj = {}
    if($stateParams.id){
        $scope.title = '编辑问题'
        $http
          .get($scope.baseURL + '/api/helpCenter/detail?platform=1&id=' + $stateParams.id)
          .success(function(rs){
            if(rs.code == 0){

...
- (void)setupImage {
    ...
    if (self.isURL) {
        if (self.imagesArray.count==1) {
            [self.leftImageView sd_setImageWithURL:[self pathToURL:self.index] placeholderImage:self.placeImage];
        ...
        if (self.imagesArray.count==2&&self.index==0) {
            self.imageScrollView.bounces = YES;
        }
       // NSLog(@"!!!!!!!!!!!!!%ld",self.index);
    }else {
...
- (UIScrollView *)imageScrollView {
    ...
    if (_imageScrollView == nil) {
        ...
        if (self.imagesArray.count>0) {
         _imageScrollView.contentSize = CGSizeMake(W * self.imagesArray.count, H);
        }

...
{
    [self presentPopupView:popupViewController.view animationType:animationType isHaveBack:YES isHaveBackView:YES];
}

- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType
{
    UIView *sourceView = [self mjTopView];
...
{
    UIView *sourceView = [self mjTopView];
    UIView *popupView = view;
    UIView *overlayView = view.superview;
...
- (void)presentPopupView:(UIView*)popupView animationType:(MJPopupViewAnimation)animationType
    ...
    if (isHaveBackView) {
        backgroundView = [[MJPopupBackgroundView alloc] initWithFrame:sourceView.bounds];
    }else {
        backgroundView = [[UIView alloc] initWithFrame:sourceView.bounds];
    }
...
- (void)slideViewIn:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView withAnimationType:(MJPopupViewAnimation)animationType
{
    ...
    [UIView animateWithDuration:kPopupModalAnimationDuration delay:0.0f options:UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)slideViewOut:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView withAnimationType:(MJPopupViewAnimation)animationType
{
    ...
    [UIView animateWithDuration:kPopupModalAnimationDuration delay:0.0f options:UIViewAnimationOptionAllowUserInteraction animations:^{
        popupView.frame = popupEndRect;
        backgroundView.alpha = 0.0f;
    } completion:^(BOOL finished) {

...
public class DepartmentServiceImpl extends SchedulCommon implements DepartmentService {
    ...
    @Override
    public List<DoctorInfoDTO> getScheduleByTime(List<ScheduleNumInfo> scheduleNumInfos) {
        List<DoctorInfoDTO> list = new ArrayList<>();
        try {
            for (ScheduleNumInfo scheduleNumInfo : scheduleNumInfos) {
    ...
    @Override
    public List<DoctorInfoDTO> queryScheduleList(String hospitalCode, String hosDeptCode, String area) {
        long current = new Date().getTime();
        List<ScheduleNumInfo> scheduleNumInfos = queryAllSchedule(hospitalCode, hosDeptCode, area);
        if (null == scheduleNumInfos) return null;
        Map<String, ScheduleNumInfo> map = new HashMap<>();
        for (ScheduleNumInfo scheduleNumInfo : scheduleNumInfos) {
            if (scheduleNumInfo.getTimeRange().equals("1") || scheduleNumInfo.getTimeRange().equals("3")) {//app只展示上午和下午的排班
                String key = scheduleNumInfo.getHosOrgCode() + scheduleNumInfo.getHosDeptCode() + scheduleNumInfo.getHosDoctCode();
                if (map.get(key) == null) {
                    map.put(key, scheduleNumInfo);
                } else {
                    int orderNum = map.get(key).getOrderedNum();//已预约数
                    int reserveOrderNum = map.get(key).getReserveOrderNum();//可预约总数
                    scheduleNumInfo.setOrderedNum(scheduleNumInfo.getOrderedNum() + orderNum);
                    scheduleNumInfo.setReserveOrderNum(scheduleNumInfo.getReserveOrderNum() + reserveOrderNum);
                    map.put(key, scheduleNumInfo);
                }
        ...
        while (it.hasNext()) {
            ...
            ScheduleNumInfo value = (ScheduleNumInfo) entry.getValue();
            list.add(value);
        }
        List<DoctorInfoDTO> data = new ArrayList<>();
        for (ScheduleNumInfo info : list) {
            Doctor doctor = doctorService.synchronDoctor(info.getHosOrgCode(), info.getHosDeptCode(), info.getHosDoctCode());
            if (doctor == null) continue;//本地查询不到医生信息不展示其排班信息
            if (info.getReserveOrderNum() > info.getOrderedNum()) doctor.setIsFull(1);
            data.add(new DoctorInfoDTO(doctor));

        }
        // 按是否约满倒序
        Collections.sort(data, new Comparator<DoctorInfoDTO>() {
            public int compare(DoctorInfoDTO arg0, DoctorInfoDTO arg1) {
                int is0 = arg0.getIsFull();
                int is1 = arg0.getIsFull();
                if (is1 > is0) {
                    return 1;
                } else if (is1 == is0) {
                    return 0;

...
//#import "AddMyBabyViewController.h"
//#import "VaccineAssistantViewController.h"
...
- (void)presentLoginViewController:(BOOL)animation{
    if ([APP.window.rootViewController.presentationController isKindOfClass:[BaseNavigationController class]]) {
        return;
    }
    LoginViewController *loginVC = [[LoginViewController alloc] init];
    loginVC.viewModel.viewType = LoginViewTypePWD;
    loginVC.hasBack = NO;
     BaseLoginNaViewController *navController = [[BaseLoginNaViewController alloc] initWithRootViewController: loginVC];
   
//    BaseViewController *vc =  [[BFRouter router].navi.viewControllers lastObject];
//    vc.needHiddenBar = NO;
    
    [APP.window.rootViewController presentViewController: navController animated: YES completion:^{
...
- (void)showHomeViewController:(BOOL)animation {
    ...
    if (!self.rootNavController) {
        ...
        BaseNavigationController *navC = [[BaseNavigationController alloc] initWithRootViewController: rootTabbarVC];
        [APP.window setRootViewController: navC];
        self.rootNavController = navC;
    }
    [self.rootNavController setNavigationBarHidden: YES];
    [self.rootNavController setViewControllers: @[rootTabbarVC] animated: animation];
    self.rootTabController = rootTabbarVC;
}

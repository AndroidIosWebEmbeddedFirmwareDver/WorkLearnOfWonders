package com.wondersgroup.hs.healthcloud.common.huanxin.widget.chatrow;
...
import com.wondersgroup.hs.healthcloud.common.huanxin.adapter.EaseMessageAdapter;
...
//    protected ImageView userAvatarView;
    ...
    private void setUpBaseView() {
        ...
        if (timestamp != null) {
            if (position == 0) {
                timestamp.setText(EaseDateUtil.getTimestampString(new Date(message.getMsgTime())));
                timestamp.setVisibility(View.VISIBLE);
            } else {
            	// show time stamp if interval with last message is > 30 seconds
                EMMessage prevMessage = (EMMessage) adapter.getItem(position - 1);
                if (prevMessage != null && EaseDateUtil.isCloseEnough(message.getMsgTime(), prevMessage.getMsgTime())) {
        ...
        if(message.direct() == Direct.SEND){
            ...
            EaseUserUtils.setUserNick(message.getTo(), usernickView);
        }else{

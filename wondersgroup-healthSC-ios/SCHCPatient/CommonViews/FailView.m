...
#define FAILIMAGE_SIZE CGSizeMake(97, 126)
...
- (void)setupUIViews {
    ...
    _labelFail.font = [UIFont systemFontOfSize: 14];
    ...
    [_labelFail mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_imageFail.mas_bottom).offset(10);
...
- (void)showFail:(UIImage *)dataImage withTip:(NSString *)tip {
    ...
    CGSize size = [Utility heightWithAttributesWithFont:[UIFont systemFontOfSize:12] withText:tip withWidth:width withHeight:MAXFLOAT];
    
    WS(weakSelf)
    [self.imageFail mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(weakSelf);
        make.centerY.equalTo(weakSelf).offset(-(size.height+20)/2.0);

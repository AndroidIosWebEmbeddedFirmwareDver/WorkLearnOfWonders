    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        if (handler instanceof HandlerMethod) {
            ...
            if (hm.getBeanType().getAnnotation(Admin.class) != null || hm.getMethodAnnotation(Admin.class) != null) {
                if (StringUtils.isEmpty(request.getHeader("admin-token")) || StringUtils.isEmpty(request.getHeader("userId"))) {
                    buildResponse(response, -1000, "请登录后操作!");
                    return false;
                }
                Session session = sessionUtil.get(request.getHeader("admin-token"), true);
                if (!(session != null && session.getIsAdmin() && session.getIsValid() && session.getUserId().equals(request.getHeader("userId")))) {
                    buildResponse(response, -1000, "请登录后操作!");
                    return false;
                }
            }

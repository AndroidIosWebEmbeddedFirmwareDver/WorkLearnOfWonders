...
- (void)setupModel
{
    SCSettingModel *model1 = [[SCSettingModel alloc] initWithtype:SettingAccount];
    self.modelArray = @[model1];
}
...
- (void)setupHeadView
{
    ...
    iconimageView.image = [UIImage imageNamed:@"img微健康logo"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SCSettingModel *model = self.modelArray[indexPath.row];
    if (model.type == SettingAccount) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.type == SettingAccount) {
        ...
        SCAccountSetViewController *accountSet = [[SCAccountSetViewController alloc] init];
...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.viewModel = [[SCSettingViewModel alloc]init];
    }
    return self;
}

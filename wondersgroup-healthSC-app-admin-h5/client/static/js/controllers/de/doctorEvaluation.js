...
app.controller('doctorEvalutionController', ['$scope', '$http', '$state','$rootScope','$localStorage', function($scope, $http, $state,$rootScope,$localStorage){
	$scope.pager = {
		"total_pages":0, 
		"total_elements":0, 
		"size": 10, 
	...
	$scope.goPage = function(number){
		...
		pager.number = number;
		$http.post(server+'/admin/evaluate/doctor/list',pager)
		.then(function(xhr){
	...
	angular.element("#delete").on('click',function(){
		...
		layer.confirm("确定删除么？",{},function(index){
			$http.post(server + '/admin/evaluate/doctor/batchDelete',idList)
			.then(function(xhr){
	...
	$scope.goPass = function(id,statusBool,bool){
		...
		let params = null;
		let urls = {
			"status": '/admin/evaluate/doctor/updateStatus',
			"isTop": '/admin/evaluate/doctor/updateIsTop'
		},words=["通过","删除","取消置顶","置顶"];
		switch (statusBool){
			case 1: 

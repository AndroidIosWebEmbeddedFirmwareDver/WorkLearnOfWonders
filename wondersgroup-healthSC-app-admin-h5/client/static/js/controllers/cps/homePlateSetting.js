app.controller('HomeSettingCtrl', ['$scope', '$http', '$state','$filter', function($scope, $http, $state,$filter) {
    $scope.pager = {
        "totalPages":0,     //总页数
        "totalElements":0,  //总条数
        "size": 10,         //页大小
    ...
    $scope.getVersionList=function(){
        $http.get($scope.baseURL + '/api/imagetext/findGImageTextVersions?gadcode=' + 1)
    ...
    $scope.addHome = function(){
        $state.go('app.cps.homePlateSettingEdit');
    };

    $scope.editHome = function(id){
        $state.go('app.cps.homePlateSettingEdit',{ id : id,type:'edit' });
    };

    $scope.copyHome=function(id){
        $state.go('app.cps.homePlateSettingEdit',{ id:id,type:'copy' });
    };

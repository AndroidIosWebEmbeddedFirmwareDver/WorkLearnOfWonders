var idList=[]
app.controller('helpCenterController',['$scope','$http','$state',function($scope,$http,$state){
    $scope.pager={
        totalPages:1,
        totalElements:2,
        size: 20,
    ...
    $scope.goPage=function(number){
        idList=[]
        $http
          .get($scope.baseURL+'/api/helpCenter/find?page='+(number-1)+ '&platform=1')
          .success(function(rs){
              if (rs.code == 0) {
                  var  data=rs.data
                  $scope.pager={
    ...
    $scope.questionEdit = function(id){
        $state.go('app.help.helpCenterEdit',{id:id})
    }
}])
